...
runGA <- function(noRuns = 30, problem = "feature", popSize = 15, pmutation = 0.2) {
  ...
  if (problem == "feature") {
    ...
    crossover = NULL
    data <- getData()
    X <- data$X  # Extract feature matrix from the list
    y <- data$y  # Extract target variable from the list
  ...
  for (i in 1:noRuns) {
    ...
    if (problem == "feature") {
      GA <- ga(type = type, fitness = fitness, X = X, y = y, nBits = ncol(X),
               names = colnames(X), seed = i, popSize = popSize,
...
# Run with a smaller population size (20)
results2 <- runGA(noRuns = 30, problem = "feature",  popSize = 20, pmutation = 0.2)

# Run with default population size (15)
results1 <- runGA(noRuns = 30, problem = "feature", pmutation = 0.2)

# Run with a larger population size (30)
results3 <- runGA(noRuns = 30, problem = "feature",  popSize = 30, pmutation = 0.2)